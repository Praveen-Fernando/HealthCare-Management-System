...
// create a prepared statement
			String query = " insert into doctors (`D_Name`,`D_Type`,`D_Contact_Number`,`D_Address`,`D_Email`,`Hospital_ID`)"
					+ " values ( ?, ?, ?,?,?,?)";
...
// create a prepared statement
			String query = "UPDATE doctors SET D_Name=?,D_Type=?,D_Contact_Number=?,D_Address=?,D_Email=?,Hospital_ID=? WHERE Doctor_ID=?";
			PreparedStatement preparedStmt = con.prepareStatement(query);
// binding values
			preparedStmt.setString(1, dname);
			preparedStmt.setString(2, dtype);
